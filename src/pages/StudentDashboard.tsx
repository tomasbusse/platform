...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ currentUser, company }) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Welcome Section */}
      <div className="bg-gradient-to-r from-simmonds-primary to-simmonds-primary/80 p-4 sm:p-6 rounded-2xl text-white">
        <h1 className="text-xl sm:text-2xl font-bold mb-1 sm:mb-2">Welcome back, {currentUser.name}!</h1>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        <div className="bg-white p-3 sm:p-5 rounded-xl shadow-sm border border-simmonds-cream">
          <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:gap-3">
            <div className="p-2 sm:p-3 bg-simmonds-lime/20 rounded-lg">
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-xl sm:text-2xl font-bold text-simmonds-charcoal">{completedLessonsCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-3 sm:p-5 rounded-xl shadow-sm border border-simmonds-cream">
          <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:gap-3">
            <div className="p-2 sm:p-3 bg-simmonds-primary/10 rounded-lg">
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-xl sm:text-2xl font-bold text-simmonds-charcoal">{inProgressCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-3 sm:p-5 rounded-xl shadow-sm border border-simmonds-cream">
          <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:gap-3">
            <div className="p-2 sm:p-3 bg-simmonds-terracotta/20 rounded-lg">
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-xl sm:text-2xl font-bold text-simmonds-charcoal">{upcomingLessons.length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-simmonds-cream overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-simmonds-cream">
            <h2 className="text-base sm:text-lg font-semibold text-simmonds-charcoal flex items-center gap-2">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-simmonds-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              Upcoming Lessons
            </h2>
          ...
          <div className="divide-y divide-simmonds-cream">
            {upcomingLessons.length === 0 ? (
              <div className="p-6 sm:p-8 text-center text-simmonds-stone">
                <svg className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-3 text-simmonds-cream" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <p className="text-sm sm:text-base">No upcoming lessons scheduled</p>
              </div>
            ) : (
              upcomingLessons.slice(0, 5).map((lesson) => (
                <div key={lesson._id} className="p-3 sm:p-4 hover:bg-simmonds-cream/30 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-medium text-simmonds-charcoal text-sm sm:text-base">{lesson.title}</h3>
                    <span className={`text-xs px-2 py-0.5 sm:py-1 rounded-full ${getLevelColor(lesson.level)}`}>
                      {lesson.level}
                    </span>
                  </div>
                  <p className="text-xs sm:text-sm text-simmonds-stone mb-2">{lesson.topic}</p>
                  <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-simmonds-stone">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-simmonds-cream overflow-hidden">
          ...
          <div className="divide-y divide-simmonds-cream">
            {!virtualLessons || virtualLessons.length === 0 ? (
              <div className="p-6 sm:p-8 text-center text-simmonds-stone">
                ...
                <p className="text-sm sm:text-base">No virtual lessons available yet</p>
              </div>
            ) : (
              virtualLessons.slice(0, 5).map((lesson) => {
                ...
                return (
                  <div key={lesson._id} className="p-3 sm:p-4 hover:bg-simmonds-cream/30 transition-colors cursor-pointer">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-medium text-simmonds-charcoal text-sm sm:text-base">{lesson.title}</h3>
                      <span className={`text-xs px-2 py-0.5 sm:py-1 rounded-full ${getLevelColor(lesson.level)}`}>
                        {lesson.level}
                    ...
                    <p className="text-xs sm:text-sm text-simmonds-stone mb-3">{lesson.topic}</p>
                    ...
                    <div className="mb-2">
                      ...
                      <div className="w-full bg-simmonds-cream rounded-full h-1.5 sm:h-2">
                        <div
          ...
          {virtualLessons && virtualLessons.length > 5 && (
            <div className="p-3 sm:p-4 border-t border-simmonds-cream text-center">
